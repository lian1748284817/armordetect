...
//    cout << "max_mul: " << max_mul <<endl;
 ...
 bool SerialPort::my_get_Mode(vision_tx_info_t &cardata) {
    int bytes;
    ...
    int FirstIndex = -1;
    int flag = 0;
    cout<<sizeof(vision_tx_info_t)<<endl;
    for(int i = 0; i < bytes; i++)
    {
        // std::cout<<"hex"<<(int)rdata[i]<<" ";
        //cout<<sizeof(vision_tx_info_t)<<endl;
        if( rdata[i] == 0xA5 && FirstIndex == -1&& Verify_CRC8_Check_Sum(&rdata[i], 3) && Verify_CRC16_Check_Sum(&rdata[i],sizeof(vision_tx_info_t) ))
        {
            FirstIndex = i;
            flag = 1;
        }
    ...
    // std::cout<<std::endl;
    // std::cout<<"收到的字符个数"<<bytes<<std::endl;
    // std::cout<<"size of cardata: "<< sizeof(CarData)<<std::endl;
    int max_mul = (bytes - FirstIndex) / sizeof(vision_tx_info_t);
    if(FirstIndex != -1)
    {
        if(rdata[FirstIndex + (max_mul - 1)* sizeof(vision_tx_info_t)] == 0xA5  && Verify_CRC8_Check_Sum(&rdata[FirstIndex + (max_mul - 1)* sizeof(vision_tx_info_t)], 3) )
        {
            memcpy(&cardata, &rdata[FirstIndex + (max_mul - 1) *sizeof(vision_tx_info_t)], sizeof(vision_tx_info_t));
            // std::cout << (int)cardata.my_color << " " << cardata.pitch << " " << cardata.yaw << " " << (int)cardata.is_change_target << std::endl;
            return true;
        }
        else
        {
 ...
 */
void SerialPort::my_transform_data(const vision_rx_info_t &visiondata)
{
    Tdata[0] = 0xA5;
    Tdata[1] = visiondata.mode;
    Append_CRC8_Check_Sum(Tdata, 3);

    Tdata[3] = visiondata.pitch_angle.c[0];
    Tdata[4] = visiondata.pitch_angle.c[1];
    Tdata[5] = visiondata.pitch_angle.c[2];
    Tdata[6] = visiondata.pitch_angle.c[3];

    Tdata[7] = visiondata.yaw_angle.c[0];
    Tdata[8] = visiondata.yaw_angle.c[1];
    Tdata[9] = visiondata.yaw_angle.c[2];
    Tdata[10] = visiondata.yaw_angle.c[3];

    Tdata[11]=visiondata.is_find_target;
    Tdata[12]=visiondata.armor_num;
    Tdata[13]=visiondata.is_find_buff;
    Tdata[14]=visiondata.is_hit_enable;
    Tdata[15]=visiondata.shoot_type;
    Tdata[16]=visiondata.is_spin;

    Append_CRC16_Check_Sum(Tdata, sizeof(vision_rx_info_t));
}
...
{
    result = write(fd, Tdata, sizeof(vision_rx_info_t));
//    cout<<result<<endl;
}
